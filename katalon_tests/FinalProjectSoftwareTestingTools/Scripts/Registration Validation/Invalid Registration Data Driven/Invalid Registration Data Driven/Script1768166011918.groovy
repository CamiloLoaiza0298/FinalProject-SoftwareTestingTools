...
TestData registrationData = findTestData('Data Files/registration_invalid (1)')
...
for (def index : (1..registrationData.getRowNumbers())) {
        ...
        String expected = registrationData.getValue('expected', index)
        String currentUrl = WebUI.getUrl()
        if (expected?.toLowerCase() == 'valid') {
                // Should redirect to created.html
                WebUI.verifyMatch(currentUrl, '.*created\\.html.*', true, FailureHandling.CONTINUE_ON_FAILURE)
        } else {
                // Should stay on register.html
                WebUI.verifyMatch(currentUrl, '.*register\\.html.*', true, FailureHandling.CONTINUE_ON_FAILURE)
        }
